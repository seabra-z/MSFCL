...
class TrimNet(torch.nn.Module):
    def __init__(self, in_dim, edge_in_dim, hidden_dim=64, depth=3, heads=4, dropout=0.1, outdim=3,kge_dim=128, rel_total=3):
    ...
    def get_weight(self, triples,zhongzi):
        ...
        repr_h=xall.view((1565,-1))
        # repr_h = xall.view((1258, -1))
        #print(repr_h.shape)
        np.save('data/ddinter/drug_emb_trimnet' + str(zhongzi) + '.npy', repr_h.cpu())
        # np.save('data/drugbank_2/drug_emb_trimnet' + str(zhongzi) + '.npy', repr_h.cpu())
        kge_heads = repr_h
